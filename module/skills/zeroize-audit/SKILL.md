...
See `schemas/input.json` for the full schema. Key fields:
...
| `tools/extract_compile_flags.py` | Fail fast — cannot extract per-TU flags |
| `tools/emit_ir.sh` | Fail fast — IR analysis impossible |
| `tools/emit_asm.sh` | Warn and skip assembly findings (STACK_RETENTION, REGISTER_SPILL) |
| `tools/mcp/check_mcp.sh` | Warn and treat as MCP unavailable |
| `tools/mcp/normalize_mcp_evidence.py` | Warn and use raw MCP output |
...
| `tools/validate_rust_toolchain.sh` | Warn — run preflight manually. Checks all tools, scripts, nightly, and optionally `cargo check`. Use `--json` for machine-readable output, `--manifest` to also validate the crate builds. |
| `tools/emit_rust_mir.sh` | Fail fast — MIR analysis impossible (`--opt`, `--crate`, `--bin/--lib` supported; `--out` can be file or directory) |
| `tools/emit_rust_ir.sh` | Fail fast — LLVM IR analysis impossible (`--opt` required; `--crate`, `--bin/--lib` supported; `--out` must be `.ll`) |
| `tools/emit_rust_asm.sh` | Warn and skip assembly findings (`STACK_RETENTION`, `REGISTER_SPILL`). Supports `--opt`, `--crate`, `--bin/--lib`, `--target`, `--intel-syntax`; `--out` can be `.s` file or directory. |
| `tools/diff_rust_mir.sh` | Warn and skip MIR-level optimization comparison. Accepts 2+ MIR files, normalizes, diffs pairwise, and reports first opt level where zeroize/drop-glue patterns disappear. |
| `tools/scripts/semantic_audit.py` | Warn and skip semantic source analysis |
| `tools/scripts/find_dangerous_apis.py` | Warn and skip dangerous API scan |
| `tools/scripts/check_mir_patterns.py` | Warn and skip MIR analysis |
| `tools/scripts/check_llvm_patterns.py` | Warn and skip LLVM IR analysis |
| `tools/scripts/check_rust_asm.py` | Warn and skip Rust assembly analysis (`STACK_RETENTION`, `REGISTER_SPILL`, drop-glue checks). Dispatches to `check_rust_asm_x86.py` (production) or `check_rust_asm_aarch64.py` (**EXPERIMENTAL** — AArch64 findings require manual verification). |
| `tools/scripts/check_rust_asm_x86.py` | Required by `check_rust_asm.py` for x86-64 analysis; warn and skip if missing |
| `tools/scripts/check_rust_asm_aarch64.py` | Required by `check_rust_asm.py` for AArch64 analysis (**EXPERIMENTAL**); warn and skip if missing |

**Common prerequisite:**

| Prerequisite | Failure mode if missing |
|---|---|
| `tools/generate_poc.py` | Fail fast — PoC generation is mandatory |

---

## Approved Wipe APIs

The following are recognized as valid zeroization. Configure additional entries in `configs/`.
...
- `OPENSSL_cleanse`
- `sodium_memzero`
- Volatile wipe loops (pattern-based; see `volatile_wipe_patterns` in `configs/default.yaml`)
...
The analysis pipeline uses 11 agents across 8 phases, invoked by the orchestrator (`prompts/task.md`) via `Task`. Agents write persistent finding files to a shared working directory (`/tmp/zeroize-audit-{run_id}/`), enabling parallel execution and protecting against context pressure.
...
The orchestrator reads one per-phase workflow file from `workflows/` at a time, and maintains `orchestrator-state.json` for recovery after context compression. Agents receive configuration by file path (`config_path`), not by value.
...
Analysis runs in two phases. Full commands and MCP/tool wiring: upstream `references/detection-strategy.md`.

### Phase 1 (source) — steps 1–6 *(excerpt)*

1. **Preflight build context** — valid `compile_commands.json` (C/C++) or buildable crate (`cargo check`); fail fast if not.
2. **Identify sensitive objects** — name patterns (`key`, `secret`, `seed`, …), type hints, explicit markers (configurable in `configs/default.yaml`).
3. **Detect zeroization attempts** — approved wipe APIs in scope or reachable cleanup.
4. **MCP semantic pass (when available)** — resolve symbols and cross-file references before validating wipes; skip/downgrade per `mcp_mode`.
5. **Validate wipe correctness** — length matches object size (not pointer size), ordering before `free`, coverage on normal/error paths (`NOT_ON_ALL_PATHS`, `PARTIAL_WIPE`).
6. **Data-flow and heap** — copies (`memcpy`, struct copies, by-value args) → `SECRET_COPY`; insecure heap alloc for secrets → `INSECURE_HEAP_ALLOC`.

### Phase 2 (compiler) — steps 7–12 *(outline)*

IR comparison across optimization levels for wiped stores eliminated (DSE) → `OPTIMIZED_AWAY_ZEROIZE` (**IR diff required**). Assembly for stack retention / register spills when `enable_asm`. Semantic IR / CFG steps when `enable_semantic_ir` / `enable_cfg`. *(Exact shell/Python invocations: upstream `detection-strategy.md`.)*
...
2. **`findings.json`** — Structured JSON matching `schemas/output.json` (for machine consumption and downstream tools)
...
The `findings.json` file follows the schema in `schemas/output.json`. Each `Finding` object:
...
See `schemas/output.json` for the full schema and enum values.
