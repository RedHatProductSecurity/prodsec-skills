...
Extended jq query cookbook and optional Python helpers ship in the upstream plugin (`resources/jq-queries.md`, `resources/sarif_helpers.py`). *(see upstream Trail of Bits prodsec-skills for companion files)*

The jq cookbook is inlined below. For `normalize_path`, `compute_fingerprint`, and `deduplicate_results` helpers, use the upstream `sarif_helpers.py` or reimplement from the patterns in this document.
...
## Inlined: SARIF jq query reference (upstream)
