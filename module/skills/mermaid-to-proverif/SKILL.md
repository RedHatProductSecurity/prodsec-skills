...
*(Upstream Trail of Bits **prodsec-skills**: `plugins/trailmark/skills/mermaid-to-proverif/examples/simple-handshake/`.)*
...
|--------------------|------------------|
| Long-term `keygen → sk, pk` | `new sk`; `let pk = pk(sk)`; publish `pk` on public channel |
| Ephemeral keypair | `new ek`; `let epk = dhpk(ek)` inside process |
| `DH(sk, pk_peer)` | `fun dh(skey,pkey): key` + commutativity `equation` on `dh` / `dhpk` |
| `HKDF(...)` | `fun hkdf(key, bitstring): key` + `const` labels; use `concat` or tuples as appropriate |
| `Sign` / `Verify` | `fun sign(bitstring,skey): bitstring`; `fun verify(bitstring,bitstring,pkey): bitstring` with inline `reduc` so failure aborts |
| `Enc` / `Dec` (AEAD) | Constructor + destructor with `reduc` |
| Serialize typed keys | e.g. `fun pkey2bs(pkey): bitstring` when `bitstring` needed |

Destructors that must abort on failure use inline `reduc` (not separate `equation` blocks for the same property).

### Inlined essentials (which queries)

- **Always:** reachability / sanity `query event(...)` on end-events first — if false, model is structurally broken.
- **Shared key secrecy:** `free w: bitstring [private]` + `out(c, aead_enc(w, sk_session))` + `query attacker(w)`.
- **Authentication:** weak `==>` correspondence, or **injective** `inj-event` when freshness (nonce/ephemeral/transcript) binds sessions.
- **Forward secrecy:** leak long-term keys *after* session in a variant process; past session key must stay secret — see upstream `security-properties.md` → Forward Secrecy.
- **Default set:** session key secrecy + injective authentication (both directions) when applicable; add forward secrecy if diagram shows ephemerals.

Full tables, Key Exposure Oracle, `noselect`, and `table` patterns: upstream reference files above.
