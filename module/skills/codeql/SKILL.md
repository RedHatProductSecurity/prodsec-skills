...
Then ask the user to select which database to use, or to build a new one. **Skip prompting if the user explicitly stated which database to use or to build a new one in their prompt.**
...
- **"The build fails on macOS, just use build-mode=none"** - Exit code 137 is caused by `arm64e`/`arm64` mismatch, not a fundamental build failure. See inlined macOS notes below and *(see upstream Trail of Bits prodsec-skills for companion files)* for the full workaround doc.
...
| Workflow | Purpose |
|----------|---------|
| `build-database` | Create CodeQL database using build methods in sequence *(see upstream Trail of Bits prodsec-skills for companion files)* |
| `create-data-extensions` | Detect or generate data extension models for project APIs *(see upstream Trail of Bits prodsec-skills for companion files)* |
| `run-analysis` | Select rulesets, execute queries, process results *(see upstream Trail of Bits prodsec-skills for companion files)* |
...
| One database found | Ask the user: reuse it or build new? |
| Multiple databases found | Ask the user: list all with metadata, let user pick one or build new |
| User explicitly stated intent | Skip database prompt, act on their instructions directly |

### Database Selection Prompt

When existing databases are found **and the user did not explicitly specify which to use**, present options such as:

```
Existing CodeQL Databases
Question: I found existing CodeQL database(s). What would you like to do?
Options:
  - <db_path_1> (language: python, created: 2026-02-24) — Reuse this database
  - <db_path_2> (language: cpp, created: 2026-02-23) — Reuse this database
  - Build a new database — Create a fresh database in a new output directory
```
...
Upstream plugin files (same names) contain full detail. *(see upstream Trail of Bits prodsec-skills for companion files)*

| Area | Upstream files (indicative) |
|------|----------------------------|
| Workflows | `workflows/build-database.md`, `workflows/create-data-extensions.md`, `workflows/run-analysis.md` |
| macOS / build | `references/macos-arm64e-workaround.md`, `references/build-fixes.md` |
| Quality / extensions | `references/quality-assessment.md`, `references/extension-yaml-format.md` |
| Analysis output | `references/sarif-processing.md`, `references/diagnostic-query-templates.md` |
| Suites / packs | `references/important-only-suite.md`, `references/run-all-suite.md`, `references/ruleset-catalog.md` |
| Other | `references/threat-models.md`, `references/language-details.md`, `references/performance-tuning.md` |
...
## Inlined essentials (from upstream prodsec-skills)

### Build database workflow (summary)

- **Interpreted** (Python, JS, Go, Ruby): no build; use `codeql database create` with `--codescanning-config` exclusion file when applicable.
- **Compiled** (C/C++, Java, C#, Rust, Swift): trace the real build. Try in order: autobuild → custom command → **macOS arm64e workaround** (Homebrew `arm64` compiler + multi-step tracing) → multi-step trace-command → **`build-mode=none` only as last resort**.
- **macOS arm64e:** System tools may be `arm64e` while CodeQL’s `libtrace.dylib` is not, causing SIGKILL (exit 137). Detect and route to Homebrew toolchain / Rosetta before giving up.
- Maintain `$OUTPUT_DIR/build.log` with commands and outcomes.

### Run analysis workflow (summary)

- **Never** pass raw pack directories to `analyze` without an explicit `.qls` suite — hidden `defaultSuiteFile` filters can yield zero results.
- **Modes:** “Run all” ≈ `security-and-quality` + `security-experimental`; “Important only” uses a filtered security-focused suite *(full templates upstream)*.
- **Gates:** confirm scan mode, query pack selection, and threat model with the user unless already specified verbatim in the request.
- Discover/install Trail of Bits and community packs per language where applicable *(catalog upstream)*.

### Create data extensions workflow (summary)

1. Search for existing model packs / `extensions:` YAML in-repo and via `codeql resolve qlpacks`.
2. If none, run source/sink enumeration queries against the DB *(QL templates upstream)* → CSVs under `$OUTPUT_DIR/diagnostics/`.
3. Identify gaps; author extension YAML under `$OUTPUT_DIR/extensions/` *(column format upstream)*.
4. Re-run analysis to validate improved coverage.

### Quality assessment (metrics snippet)

```bash
log_step()   { echo "[$(date -Iseconds)] $1" >> "$LOG_FILE"; }
log_result() { echo "[$(date -Iseconds)] RESULT: $1" >> "$LOG_FILE"; }
...
Compare baseline LoC and project file counts to expectations; investigate high extractor error ratios before trusting results.
